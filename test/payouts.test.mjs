...
test('writeOffAllBelowMin: closes a genuinely below-minimum clip at zero and leaves a reopen trail', async () => {
  const now = Date.now();
  const db = makePayoutsDb({
    campaigns: [campaign()],
    submissions: [sub({ id: 1, views: 400, last_ok_sync_at: now, earning: 0 })]
  });
...
test('writeOffAllBelowMin: never touches a clip that has not been synced yet', async () => {
  const db = makePayoutsDb({
    campaigns: [campaign()],
    submissions: [sub({ id: 1, views: 0, last_ok_sync_at: null })]
  });
...
test('writeOffAllBelowMin: never touches a clip that already qualifies and is earning money', async () => {
  ...
  const db = makePayoutsDb({
    campaigns: [campaign()],
    submissions: [sub({ id: 1, views: 5000, last_ok_sync_at: now, earning: 200 })]
  });
  const result = await writeOffAllBelowMin(db);
  assert.equal(result.closed, 0);
...
test('writeOffAllBelowMin: never re-locks a clip that is already locked (paid or previously written off)', async () => {
  ...
  const db = makePayoutsDb({
    campaigns: [campaign()],
    submissions: [sub({ id: 1, views: 400, last_ok_sync_at: now, locked_at: now - DAY_MS, locked_earning: 0, lock_reason: 'below_min' })]
  });
  const result = await writeOffAllBelowMin(db);
...
test('writeOffAllBelowMin: campaign/clipper filters scope the sweep correctly', async () => {
  ...
  const db = makePayoutsDb({
    ...
    submissions: [
      sub({ id: 1, clipper_id: 1, campaign_id: 1, views: 400, last_ok_sync_at: now }),
      sub({ id: 2, clipper_id: 2, campaign_id: 2, views: 400, last_ok_sync_at: now })
    ]
  });
...
test('writeOffAllBelowMin: multiple below-min clips across different campaigns are all closed in one sweep, unscoped', async () => {
  const now = Date.now();
  const db = makePayoutsDb({
    campaigns: [campaign({ id: 1 }), campaign({ id: 2 })],
    submissions: [
      sub({ id: 1, clipper_id: 1, campaign_id: 1, views: 400, last_ok_sync_at: now }),
      sub({ id: 2, clipper_id: 2, campaign_id: 2, views: 100, last_ok_sync_at: now }),
      sub({ id: 3, clipper_id: 3, campaign_id: 2, views: 9000, last_ok_sync_at: now, earning: 300 }) // stays open
    ]
  });
  const result = await writeOffAllBelowMin(db);
